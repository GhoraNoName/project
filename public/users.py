...
from models.models import Main_User, Main_UserDB, New_Response
...
users_list = [
    Main_UserDB(name="Летов", id=144, password="**********")]
...
def create_user(item: Annotated[Main_User, Body(embed=True, description="Новый пользователь")]):
    user = Main_User(name=item.name, id=item.id, password=coder_passwd(item.name))
